...
- **Promote `PositionMove` and retire the `Move` alias** — Phase 3 introduces `PositionMove` as the canonical name for position-based moves, with `Move` kept as a backward-compat alias. A follow-up session should replace all existing `Move` usages with `PositionMove` and delete the alias. Similarly rename `AnyFace` → `PositionalFace`. Do as a standalone mechanical rename session (~20+ files).
