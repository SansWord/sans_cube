...
- **Rename `Move` → `PositionMove` and `AnyFace` → `PositionalFace`** — now that `ColorMove` and `FaceletColor` exist, `Move` and `AnyFace` are ambiguous by comparison. A full rename makes the position-vs-color distinction explicit across the codebase. Deferred because `Move` is used in ~20+ files; do as a standalone mechanical rename session.
