...
## firebase
- **Anon → Google account merging on sign-in** — when an anonymous user (created on first Share click; see v1.31.0 public-sharing flow) later signs in with Google, Firebase issues a *third* uid; the previous anon shares stay owned by the anon uid and become unmanageable from the Google account. Designing a migration would mean either (a) using `linkWithCredential` to upgrade the anon account in place, or (b) re-stamping ownership of each `users/{anonUid}/shared_solves/{shareId}` doc to the new Google uid (and the corresponding `solve.shareId` records). Defer until there's evidence users hit this in practice.
- **Detect un-unshare-able shares and hide the Unshare button** — v1.31.0 shipped with a simple gate: Unshare is shown whenever any user is signed in. This misses two cases where clicking Unshare will fail with a Firestore rule rejection: (a) shared while anonymous, then site data cleared (anon uid gone, but `solve.shareId` still on localStorage), and (b) shared while anonymous, then signed in with Google (different uid, doesn't own the share). Fix: stamp `shareOwnerUid` on the local `SolveRecord` at share time, then in `SolveDetailModal` hide Unshare unless `cloudConfig.user.uid === solve.shareOwnerUid`. Legacy shares (pre-v1.31.0, no stamp) fall through to current behavior. Adds one optional field; no migration. Also lays groundwork for the anon→Google merge item above (we'd know which uid originally owned each share).
