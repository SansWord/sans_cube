// Generic 3x3 cube geometry. Facelet positions in the Kociemba 54-char string
// are spatially fixed; what drifts under M/E/S slice moves or whole-cube
// rotations is which color sits at each center. Checks that used to compare
// against hardcoded letters ('W', 'Y', 'O', ...) should compare against live
// center values (f[center]) so they keep working when centers drift.

export const U_CENTER = 4
export const R_CENTER = 13
export const F_CENTER = 22
export const D_CENTER = 31
export const L_CENTER = 40
export const B_CENTER = 49

export const FACE_CENTERS = [U_CENTER, R_CENTER, F_CENTER, D_CENTER, L_CENTER, B_CENTER] as const

export const FACE_POSITIONS: Record<number, number[]> = {
  [U_CENTER]: [0, 1, 2, 3, 4, 5, 6, 7, 8],
  [R_CENTER]: [9, 10, 11, 12, 13, 14, 15, 16, 17],
  [F_CENTER]: [18, 19, 20, 21, 22, 23, 24, 25, 26],
  [D_CENTER]: [27, 28, 29, 30, 31, 32, 33, 34, 35],
  [L_CENTER]: [36, 37, 38, 39, 40, 41, 42, 43, 44],
  [B_CENTER]: [45, 46, 47, 48, 49, 50, 51, 52, 53],
}

export const OPPOSITE: Record<number, number> = {
  [U_CENTER]: D_CENTER,
  [D_CENTER]: U_CENTER,
  [F_CENTER]: B_CENTER,
  [B_CENTER]: F_CENTER,
  [R_CENTER]: L_CENTER,
  [L_CENTER]: R_CENTER,
}

// Each edge: [pos on side A, center of side A, pos on side B, center of side B]
export type Edge = [number, number, number, number]
export const EDGES: Edge[] = [
  [7,  U_CENTER, 19, F_CENTER],  // UF
  [5,  U_CENTER, 10, R_CENTER],  // UR
  [1,  U_CENTER, 46, B_CENTER],  // UB
  [3,  U_CENTER, 37, L_CENTER],  // UL
  [23, F_CENTER, 12, R_CENTER],  // FR
  [21, F_CENTER, 41, L_CENTER],  // FL
  [48, B_CENTER, 14, R_CENTER],  // BR
  [50, B_CENTER, 39, L_CENTER],  // BL
  [28, D_CENTER, 25, F_CENTER],  // DF
  [32, D_CENTER, 16, R_CENTER],  // DR
  [34, D_CENTER, 52, B_CENTER],  // DB
  [30, D_CENTER, 43, L_CENTER],  // DL
]

// Each corner: 3 pairs of [sticker pos, owning face center]
export type Corner = [[number, number], [number, number], [number, number]]
export const CORNERS: Corner[] = [
  [[8,  U_CENTER], [20, F_CENTER], [9,  R_CENTER]],  // UFR
  [[6,  U_CENTER], [18, F_CENTER], [38, L_CENTER]],  // UFL
  [[2,  U_CENTER], [45, B_CENTER], [11, R_CENTER]],  // UBR
  [[0,  U_CENTER], [47, B_CENTER], [36, L_CENTER]],  // UBL
  [[29, D_CENTER], [26, F_CENTER], [15, R_CENTER]],  // DFR
  [[27, D_CENTER], [24, F_CENTER], [44, L_CENTER]],  // DFL
  [[35, D_CENTER], [51, B_CENTER], [17, R_CENTER]],  // DBR
  [[33, D_CENTER], [53, B_CENTER], [42, L_CENTER]],  // DBL
]

export function edgesOnFace(centerIdx: number): Edge[] {
  return EDGES.filter(([, c1, , c2]) => c1 === centerIdx || c2 === centerIdx)
}

export function cornersOnFace(centerIdx: number): Corner[] {
  return CORNERS.filter((c) => c.some(([, ctr]) => ctr === centerIdx))
}

export function findFaceWithColor(f: string, color: string): number | undefined {
  return FACE_CENTERS.find((c) => f[c] === color)
}
